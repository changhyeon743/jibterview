...
export class ParametricsInterface {
  constructor(parametricDataClass, viewer3D, x = 0, y = 0, appParent = null) {
    ...
    this.__settings = QuickSettings.create(
      x,
      y,
      `Parametric Item: ${parametricDataClass.name}`,
      appParent,
    );
    this.__constructUI();
  }
  ...
  __constructUI() {
    ...
    for (let opt in parameters) {
      ...
      switch (property.type) {
        case "color":
          this.__settings.addColor(
            opt,
            parametricsClass[opt],
            function (value) {
