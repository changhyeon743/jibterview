...
export class Material3D extends MeshPhysicalMaterial {
  constructor(parameters, textureMapPack, scene, reflectsScene = false) {
    ...
    this.__repeat = !textureMapPack.repeat
      ? textureMapPack.repeat
      : textureMapPack.repeat == 0
        ? 0
        : TEXTURE_DEFAULT_REPEAT;
    this.__reflective = textureMapPack.reflective
      ? textureMapPack.reflective
      : textureMapPack.reflective == 0
        ? 0
        : TEXTURE_DEFAULT_REFLECTIVE;
    this.__shininess = textureMapPack.shininess
      ? textureMapPack.shininess
      : textureMapPack.shininess == 0
        ? 0
        : TEXTURE_DEFAULT_SHININESS;
  ...
  __updateNormalMap(texture) {
    ...
    if (this.__normalTexture) {
      if (this.__normalTexture.image) {
        ...
        this.__normalTexture.wrapS = this.__normalTexture.wrapT =
          RepeatWrapping;
  ...
  __updateRoughnessMap(texture) {
    if (this.__roughnessTexture) {
      if (this.__roughnessTexture.image) {
        ...
        this.__roughnessTexture.wrapS = this.__roughnessTexture.wrapT =
          RepeatWrapping;
  ...
  __updateAmbientMap(texture) {
    if (this.__ambientTexture) {
      if (this.__ambientTexture.image) {
        ...
        this.__ambientTexture.wrapS = this.__ambientTexture.wrapT =
          RepeatWrapping;
  ...
  __applyNewTextures() {
    ...
    if (this.__textureMapPack.colormap) {
      this.__colorTexture = new TextureLoader().load(
        this.__textureMapPack.colormap,
        this.__updateColorMap.bind(this),
      );
    }
    if (this.__textureMapPack.normalmap) {
      this.__normalTexture = new TextureLoader().load(
        this.__textureMapPack.normalmap,
        this.__updateNormalMap.bind(this),
      );
    }
    if (this.__textureMapPack.roughnessmap) {
      this.__roughnessTexture = new TextureLoader().load(
        this.__textureMapPack.roughnessmap,
        this.__updateRoughnessMap.bind(this),
      );
    }
    if (this.__textureMapPack.ambientmap) {
      this.__ambientTexture = new TextureLoader().load(
        this.__textureMapPack.ambientmap,
        this.__updateAmbientMap.bind(this),
      );
    }
    if (this.__textureMapPack.metalmap) {
      this.__metalTexture = new TextureLoader().load(
        this.__textureMapPack.metalmap,
        this.__updateMetallicMap.bind(this),
      );
    }
      ...
      if (o.isMesh) {
        ...
        if (obj) {
          if (obj.texture != "") {
            ...
            let INITIAL_MTL = new MeshStandardMaterial({
              map: txt,
              flatShading: true,
            });
