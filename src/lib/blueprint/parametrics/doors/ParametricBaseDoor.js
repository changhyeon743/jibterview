...
export const DOOR_HANDLE_TYPES = Enum(
  "None",
  "HANDLE_01",
  "HANDLE_02",
  "HANDLE_03",
...
export class ParametricBaseDoor extends EventDispatcher {
  constructor(parameters) {
    ...
    this.__frameMaterial = new Material3D({
      color: opts.frameColor,
      side: DoubleSide,
    });
    this.__doorMaterial = new Material3D({
      color: opts.doorColor,
      side: DoubleSide,
      wireframe: false,
    });
    this.__handleMaterial = new Material3D({
      color: "#F0F0FF",
      side: DoubleSide,
      wireframe: false,
    });
    ...
    this.__doorHandleMaterial = new Material3D({
      color: opts.doorHandleColor,
      wireframe: false,
      roughness: 0.0,
      metalness: 0.0,
    });
    this.__glassMaterial = new Material3D({
  ...
  __convertFrom4ToFace3(facegroups, vertices, materialId = 0) {
    ...
    for (let i = 0; i < facegroups.length; i += 4) {
      ...
      // faces.push(f2);
      faces.push(
        vertices[facegroups[i]],
        vertices[facegroups[i + 1]],
        vertices[facegroups[i + 2]],
      );
      faces.push(
        vertices[facegroups[i]],
        vertices[facegroups[i + 2]],
        vertices[facegroups[i + 3]],
      );
    }
    return faces;
  }
  ...
  __proceedure() {
    ...
    if (doorGeometries.length) {
      ...
      if (doorGeometries[1]) {
        tempGeometry = mergeBufferGeometries(
          [tempGeometry, doorGeometries[1]],
          true,
        ); //Maintain all door geometries as same group for material assignment purposes
      }
  ...
  __createDoorFrame() {
    ...
    let faceIds = [
      3, 4, 1, 0, 7, 12, 19, 9, 4, 3, 6, 5, 10, 11, 5, 6, 13, 20, 21, 14, 17,
      15, 16, 18, 11, 23, 22, 5, 20, 13, 12, 7, 20, 3, 0, 21, 9, 10, 6, 7, 13,
      14, 16, 15, 4, 8, 2, 1, 29, 30, 27, 31, 7, 6, 3, 20, 8, 4, 5, 22, 14, 2,
      18, 16, 17, 18, 2, 8, 28, 25, 19, 12, 28, 26, 24, 25, 25, 24, 23, 19, 22,
      23, 24, 26, 29, 31, 26, 17, 15, 28, 27, 30,
    ];
    ...
    geometry.applyMatrix4(
      new Matrix4().makeRotationAxis(new Vector3(1, 0, 0), -Math.PI * 0.5),
    );
    geometry.applyMatrix4(
      new Matrix4().makeTranslation(0, -this.__frameHeight * 0.5, 0),
    );
  ...
  __shapeChildren() {
    ...
    switch (this.__openDirection) {
      ...
      case DOOR_OPEN_DIRECTIONS.LEFT:
        ...
        doorLeft = this.__makeOneDoor(
          w,
          this.__openDirection,
          this.__leftDoorMaterialId,
          "Left",
        );
        doorLeft.applyMatrix4(
          new Matrix4().makeTranslation(
            -(this.__frameWidth * 0.5) + this.__frameSize,
            0,
            0,
          ),
        );
      ...
      case DOOR_OPEN_DIRECTIONS.RIGHT:
        //Get the buffergeometry of only one door
        doorRight = this.__makeOneDoor(
          w,
          this.__openDirection,
          this.__rightDoorMaterialId,
          "Right",
        );
        doorRight.applyMatrix4(
          new Matrix4().makeTranslation(
            this.__frameWidth * 0.5 - this.__frameSize,
            0,
            0,
  ...
  __makeOneDoor(
    ...
    let aDoorGeometry = this.__createDoorData(
      frameWidth,
      openingDirection,
      materialId,
    );
    ...
    if (this.__handleType !== DOOR_HANDLE_TYPES.None) {
      let doorRatio =
        doorSide === "Right" ? 1.0 - this.doorRatio : this.doorRatio;
      let front_handle = DoorHandleGenerator.generate_handle(
        this.__handleType.description,
        "Front",
        doorSide,
        doorRatio,
        this.frameWidth,
        this.frameSize,
        this.frameThickness,
        this.__openDirection.description,
        this.__doorHandleMaterialId,
      );
      let back_handle = DoorHandleGenerator.generate_handle(
        this.__handleType.description,
        "Back",
        doorSide,
        doorRatio,
      ...
      aDoorGeometry = mergeBufferGeometries(
        [aDoorGeometry, front_handle, back_handle],
        true,
      );
  ...
  __createDoorData(frameWidth, openingDirection, materialId = 1) {
    let doorModelData = this.__createForDoorModel(
      frameWidth,
      openingDirection,
      materialId,
    );
  ...
  __createForDoorModel(frameWidth, openingDirection, materialId = 1) {
    ...
    let deep = this.__frameThickness * 0.5 - gap * 3;
    let side = 0,
      minx = 0,
      maxx = 0;
    ...
    let faceids = [
      4, 5, 1, 0, 5, 6, 2, 1, 6, 7, 3, 2, 7, 4, 0, 3, 0, 1, 2, 3, 7, 6, 5, 4,
    ];
    // # Vertex
    let vertices = [
