...
export class DragRoomItemsControl3D extends EventDispatcher {
  ...
  __updateMouse(evt) {
    ...
    // Adjust mouse coordinates based on camera type
    if (this.__camera instanceof OrthographicCamera) {
      // For orthographic camera, use normalized device coordinates
      this.__mouse.x = ((evt.clientX - rect.left) / rect.width) * 2 - 1;
      this.__mouse.y = -((evt.clientY - rect.top) / rect.height) * 2 + 1;
    } else {
      // For perspective camera, use screen coordinates
      this.__mouse.x = evt.clientX - rect.left;
      this.__mouse.y = evt.clientY - rect.top;
    }

    this.__mouseClient.x = this.__mouse.x;
    this.__mouseClient.y = this.__mouse.y;
  }
  ...
  __pressListener(evt) {
    ...
    wallPlanesThatIntersect = this.__raycaster.intersectObjects(
      this.__walls,
      false,
    );
    floorPlanesThatIntersect = this.__raycaster.intersectObjects(
      this.__floors,
      false,
    );
    this.__intersections = this.__raycaster.intersectObjects(
    ...
    if (this.__intersections.length) {
      ...
      if (
        ...
        this.__inverseMatrix = this.__selected.parent.matrixWorld
          .clone()
          .invert();
        /**
        ...
        this.__offset.copy(
          this.__intersection.clone().sub(this.__selected.location),
        );
      }
      ...
      if (deltaTime < 300) {
        if (wallPlanesThatIntersect.length) {
          ...
          this.dispatchEvent({
            ...
            normal: floorPlanesThatIntersect[0].face.normal
              .clone()
              .applyMatrix4(floorPlanesThatIntersect[0].object.matrixWorld),
          });
        }
      }
    }
  ...
  __moveListener(evt) {
    ...
    if (this.__selected && this.__enabled && this.__selected.visible) {
      ...
      if (
        this.__selected.intersectionPlanes != undefined &&
        !this.__selected.intersectionPlanes.length
      ) {
        if (
          this.__raycaster.ray.intersectPlane(this.__plane, this.__intersection)
        ) {
          ...
          location = this.__selected.location
            .clone()
            .copy(this.__intersection.sub(this.__offset)); //.applyMatrix4(this.__inverseMatrix));
        ...
        customIntersectingPlanes = customIntersectingPlanes.concat(
          customIntersectingPlanes1,
        );
        ...
        if (customPlanesThatIntersect.length) {
          ...
          if (Configuration.getBooleanValue(snapToGrid)) {
            intersectionData.point.x =
              Math.floor(
                intersectionData.point.x /
                  Configuration.getNumericValue(snapTolerance3D),
              ) * Configuration.getNumericValue(snapTolerance3D);
            intersectionData.point.y =
              Math.floor(
                intersectionData.point.y /
                  Configuration.getNumericValue(snapTolerance3D),
              ) * Configuration.getNumericValue(snapTolerance3D);
            intersectionData.point.z =
              Math.floor(
                intersectionData.point.z /
                  Configuration.getNumericValue(snapTolerance3D),
              ) * Configuration.getNumericValue(snapTolerance3D);
