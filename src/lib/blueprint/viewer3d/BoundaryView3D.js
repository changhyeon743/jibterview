...
export class BoundaryView3D extends Mesh {
  ...
  __buildFloor() {
    ...
    let material = new FloorMaterial3D(
      { color: this.__boundary.style.color, side: DoubleSide },
      this.__boundary.style,
      this.__scene,
    );
    let useGeometry = geometry; //new BufferGeometry().fromGeometry(geometry);

    material.dimensions = new Vector2(
      this.__boundary.width,
      this.__boundary.height,
    );
