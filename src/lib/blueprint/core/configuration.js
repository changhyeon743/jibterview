...
export const snapTolerance3D = "snapTolerance3D"; //In CMS
...
export const cornerTolerance = 25;
  ...
  static setValue(key, value) {
    if (key == configDimUnit) {
      if (!availableDimUnits.includes(value)) {
        throw new Error(
          `Unknown dimensional units option: ${value}. Possible values are ${availableDimUnits}`,
        );
      }
    }
    config[key] = value;
