...
export class Model extends EventDispatcher {
  ...
  loadSerialized(json) {
    ...
    this.newDesign(
      data.floorplanner || data.floorplan,
      data.items,
      data.lights,
      data.environment,
    );
  ...
  addItemByMetaData(metadata) {
    ...
    let ItemClass = item_types[itemMetaData.itemType] || FloorItem;
    let item = new ItemClass(itemMetaData, this);

    if (ItemClass === FloorItem) {
      this.__floorItems.push(item);
    }
  ...
  rotateItem(item, x, y, z) {
     ...
     */
    let eulerAngle = item.itemModel.innerRotation
      .clone()
