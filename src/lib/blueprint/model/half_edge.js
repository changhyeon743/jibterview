...
export class HalfEdge extends EventDispatcher {
  ...
  __generateEdgePlane(interior = true, plane = null) {
    ...
    this.__mathPlane.setFromNormalAndCoplanarPoint(
      this.__normal.clone(),
      this.__vertices[0].clone(),
    );

    points = [v3, v2, v1, v4, v3, v1];
    ...
    if (!plane) {
      plane = new Mesh(
        bufferGeometry,
        new MeshStandardMaterial({ visible: true }),
      );
    } else {
      plane.geometry.dispose();
      plane.geometry = bufferGeometry; //this.__plane.geometry.fromGeometry(geometry);
    }
  ...
  computeTransforms(transform, invTransform, start, end) {
    ...
    invTransform.copy(transform.clone().invert());
  ...
  interiorPointByEdges(v1, v2, debug = false) {
    ...
    let startV1 = v1
      .getStart()
      .location.clone()
      .add(u.clone().negate().multiplyScalar(multiplier));
    let endV1 = v1
      .getEnd()
      .location.clone()
      .add(u.clone().multiplyScalar(multiplier));

    let startV2 = v2
      .getStart()
      .location.clone()
      .add(v.clone().negate().multiplyScalar(multiplier));
    let endV2 = v2
      .getEnd()
      .location.clone()
      .add(v.clone().multiplyScalar(multiplier));
    ...
    let intersection = Utils.lineLineIntersectPoint(
      lineAStart,
      lineAEnd,
      lineBStart,
      lineBEnd,
    ...
    if (!intersection) {
      return orthoU
        .normalize()
        .multiplyScalar(this.wall.thickness - WALL_OFFSET_THICKNESS);
    }
  ...
  interiorPointByEdges_V2(v1, v2, debug = false) {
    ...
    if (dot > 1.0 - 1e-6 || dot < 1e-6 - 1.0) {
      w = f
        .clone()
        .normalize()
        .multiplyScalar(Math.min(v2.wall.thickness, v1.wall.thickness));
    ...
    magnitude =
      (f.length() ** 2 +
        g.length() ** 2 +
        2 * f.length() * g.length() * abs_dot) **
      0.5;
    theta = Math.asin((g.length() * Math.sin(abs_dot_acos)) / magnitude);
    w = f
      .clone()
      .rotateAround(new Vector2(), -theta)
      .normalize()
  ...
  interiorPointByEdgesOLD(v1, v2) {
    ...
    if (!v1 || !v2) {
      if (!v1) {
        ...
        directionSelf = this.getStart()
          .location.clone()
          .sub(this.getEnd().location)
          .normalize();
        directionSelf = directionSelf.multiplyScalar(this.wall.thickness);
        directionSelf = directionSelf.rotateAround(new Vector2(), 0.785398); //Rotate by 45 degrees CW
      }
  ...
  interiorPoint(nextOrPrev, isFromStart = true) {
    ...
    if (isFromStart) {
      ...
      directionOther = nextOrPrev
        .getStart()
        .location.clone()
        .sub(nextOrPrev.getEnd().location)
        .normalize();
    } else {
      directionSelf = this.getStart()
        .location.clone()
      ...
      directionOther = nextOrPrev
        .getEnd()
        .location.clone()
        .sub(nextOrPrev.getStart().location)
        .normalize();
    }
  ...
  destroy() {
    ...
    this.wall.removeEventListener(
      EVENT_ITEM_REMOVED,
      this.__wallItemChangesEvent,
    );
