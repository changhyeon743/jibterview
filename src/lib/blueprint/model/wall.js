...
export class Wall extends EventDispatcher {
  ...
  constructor(start, end, aa, bb) {
    ...
    this.__cornerAttributesChangedEvent =
      this.__cornerAttributesChanged.bind(this);
  ...
  projectOnWallPlane(pt2d) {
    ...
    projected3D = this.__wallPlane2D.projectPoint(
      new Vector3(pt2d.x, pt2d.y, 0),
      projected3D,
    );
  ...
  updateControlVectors() {
    ...
    this.__wallPlane2D = this.__wallPlane2D.setFromNormalAndCoplanarPoint(
      normal,
    ...
    if (this.getStart() || this.getEnd()) {
      this.getStart() != null
        ? this.getStart().floorplan.update(false)
        : this.getEnd() != null
          ? this.getEnd().floorplan.update(false)
          : false;
    }
  ...
  wallCenter() {
    if (this.wallType === WallTypes.STRAIGHT) {
      return new Vector2(
        (this.getStart().x + this.getEnd().x) / 2.0,
        (this.getStart().y + this.getEnd().y) / 2.0,
      );
    } else if (this.wallType === WallTypes.CURVED) {
