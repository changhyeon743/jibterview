...
export class Floorplan extends EventDispatcher {
  ...
  constructor() {
    ...
    this.__cornerAttributesChangedOrMovedEvent =
      this.__cornerAttributesChangedOrMoved.bind(this);
  ...
  __wallDeleted(evt) {
    ...
    wall.removeEventListener(
      EVENT_WALL_ATTRIBUTES_CHANGED,
      this.__wallAttributesChangedEvent,
    );
  ...
  overlappedControlPoint(wall, x, y, tolerance) {
    ...
    if (
      wall.a.distanceTo(new Vector2(x, y)) < tolerance &&
      wall.wallType === WallTypes.CURVED
    ) {
      return wall.a;
    } else if (
      wall.b.distanceTo(new Vector2(x, y)) < tolerance &&
      wall.wallType === WallTypes.CURVED
    ) {
      return wall.b;
    }
  ...
  loadFloorplan(floorplan) {
    ...
    floorplan.walls.forEach((wall) => {
      ...
      if (Version.isVersionHigherThan(floorplan.version, "0.0.2a")) {
        newWall.a = wall.a;
        newWall.b = wall.b;
        if (wall.wallType && wall.wallType === "CURVED") {
  ...
  loadLockedFloorplan(floorplan) {
    ...
    for (let id in floorplan.corners) {
      ...
      let corner = new Corner(
        this,
        Dimensioning.cmFromMeasureRaw(cornerData.x),
        Dimensioning.cmFromMeasureRaw(cornerData.y),
      );
  ...
  assignOrphanEdges() {
    ...
    // give them edges
    let orphanWalls = [];
    let iterateWalls = function (wall) {}.bind(this);
  ...
  update(updateroomconfiguration = true, updatecorners = null) {
    ...
    this.__roofPlanesForIntersection.push.apply(
      this.__roofPlanesForIntersection,
      this.createRoofPlanes(),
    );
    this.__floorPlanesForIntersection.push.apply(
      this.__floorPlanesForIntersection,
      this.createFloorPlanes(),
    );
    this.__wallPlanesForIntersection.push.apply(
      this.__wallPlanesForIntersection,
      this.createWallEdgePlanes(),
    );
