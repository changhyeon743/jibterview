...
export class CornerGroup {
  ...
  __update() {
    ...
    // }
    this.__currentCenter = this.__size
      .clone()
      .multiplyScalar(0.5)
      .add(minPoint);
  ...
  __applyTransformations(scale, radians, origin) {
    ...
    let rotationMatrix = TInv.clone()
      .multiply(new Matrix4().makeRotationAxis(new Vector3(0, 0, 1), radians))
      .multiply(T);
    let scaleMatrix = TInv.clone()
      .multiply(new Matrix4().makeScale(scale.x, scale.y, 1))
      .multiply(T);
    let transformMatrix = rotationMatrix
  ...
  destroy() {
    for (let i = 0; i < this.__corners.length; i++) {
      this.__corners[i].removeEventListener(
        EVENT_MOVED,
        this.__cornerMovedEvent,
      );
    }
    this.__corners = [];
  }
...
export class CornerGroups {
  ...
  createGroups() {
    ...
    while (cornerGroups.flat().length !== this.__floorplan.corners.length) {
      let isolatedCorner = tinyNotInArray(
        this.__floorplan.corners,
        cornerGroups.flat(),
      );
      if (isolatedCorner) {
        let index = cornerGroups.push([]) - 1;
        cornerGroups[index] = tinyCornerGroups(
          isolatedCorner,
          cornerGroups[index],
        );
      } else {
