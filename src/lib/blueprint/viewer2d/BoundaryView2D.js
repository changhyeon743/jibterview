...
export class BoundaryView2D extends BaseFloorplanViewElement2D {
  constructor(floorplan, options, boundary) {
    ...
    this.__options["boundary-point-color"] = new Color(
      this.__options["boundary-point-color"],
    ).getHex();
    this.__options["boundary-line-color"] = new Color(
      this.__options["boundary-line-color"],
    ).getHex();
  ...
  __drawBoundaryRegion(pointsColor, lineColor) {
    ...
    if (this.__boundary.style.colormap) {
      ...
      let scale =
        Math.min(this.__boundary.width, this.__boundary.height) /
        this.__boundary.styleRepeat;
