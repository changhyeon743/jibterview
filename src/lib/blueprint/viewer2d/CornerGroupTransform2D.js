...
class CornerGroupRectangle extends Graphics {
  ...
  translateByPosition(position) {
    let translateMatrix = new Matrix4().makeTranslation(
      position.x,
      position.y,
      0,
    );
    ...
    for (let i = 0; i < this.__vertices.length; i++) {
      ...
      if (this.__floorplan.boundary) {
        if (
          this.__floorplan.boundary.containsPoint(
            Dimensioning.pixelToCm(co.x),
            Dimensioning.pixelToCm(co.y),
          )
        ) {
          return;
        }
        if (
          this.__floorplan.boundary.intersectsExternalDesign(
            Dimensioning.pixelToCm(co.x),
            Dimensioning.pixelToCm(co.y),
          )
        ) {
    ...
    this.__center = this.__br
      .clone()
      .sub(this.__tl)
      .multiplyScalar(0.5)
      .add(this.__tl);
    let delta = this.__center.clone().sub(this.__originalCenter);
    this.__currentTranslationMatrix = new Matrix4().makeTranslation(
      delta.x,
      delta.y,
      0,
    );
  ...
  rotateByRadians(radians) {
    let finalPoints = [];
    let T = new Matrix4().makeTranslation(
      -this.__center.x,
      -this.__center.y,
      0,
    );
    let TInv = new Matrix4().makeTranslation(
      this.__center.x,
      this.__center.y,
      0,
    );

    let rotationMatrix = new Matrix4().makeRotationAxis(
      new Vector3(0, 0, 1),
      radians,
    );
    let rotationAboutOrigin = TInv.clone().multiply(rotationMatrix).multiply(T);

    let resetRotation = new Matrix4().makeRotationAxis(
      new Vector3(0, 0, 1),
      -this.__currentRadians,
    );
    let resetRotationAboutOrigin = TInv.clone()
      .multiply(resetRotation)
      .multiply(T);
  ...
  scaleBySize(newWidth, newHeight, origin) {
    ...
    let scale = new Vector2(
      newWidth / this.__size.x,
      newHeight / this.__size.y,
    );
    ...
    let resetRotationAboutOrigin = rotateAboutOrigin.copy(
      rotateAboutOrigin.clone().invert(),
    ); //rotateAboutOrigin.clone().getInverse(rotateAboutOrigin.clone()); //Ensure to reset the rotation of currentRadians
     ...
     */
    let transformMatrix = rotateAboutOrigin
      .clone()
      .multiply(scaleAboutOrigin)
      .multiply(resetRotationAboutOrigin);
  ...
  get matrix4() {
    ...
    let rotation = new Quaternion().setFromRotationMatrix(
      this.__currentRotationMatrix,
    );
    let translation = new Vector3().setFromMatrixPosition(
      this.__currentTranslationMatrix,
    );
...
class CornerGroupTransformationPoint extends Sprite {
  constructor(svg = "/icons/rotateGroup.svg") {
    ...
    this.on("mousedown", this.__mouseDownEvent).on(
      "touchstart",
      this.__mouseDownEvent,
    );
    this.on("mouseupoutside", this.__mouseUpEvent).on(
      "touchendoutside",
      this.__mouseUpEvent,
    );
    this.on("mouseup", this.__mouseUpEvent).on("touchend", this.__mouseUpEvent);
    this.on("mousemove", this.__mouseMoveEvent).on(
      "touchmove",
      this.__mouseMoveEvent,
    );
    this.on("mouseover", this.__mouseOverEvent).on(
      "mouseout",
      this.__mouseOutEvent,
    );
  ...
  __dragMove(evt) {
    if (this.__isDragged) {
      ...
      if (snapping) {
        co.x =
          Math.floor(co.x / Configuration.getNumericValue(snapTolerance)) *
          Configuration.getNumericValue(snapTolerance);
        co.y =
          Math.floor(co.y / Configuration.getNumericValue(snapTolerance)) *
          Configuration.getNumericValue(snapTolerance);
      }
      this.__eventDispatcher.dispatchEvent({
        type: "DragMove",
...
class CornerGroupScalePoint extends Graphics {
  constructor(index, parameters) {
    ...
    this.__textfield = new Text(this.__index, {
      fontFamily: "Arial",
      fontSize: 12,
      fill: 0x000000,
      align: "center",
...
export class CornerGroupTransform2D extends Graphics {
  constructor(floorplan, parameters) {
    ...
    this.__translateHandle = new CornerGroupTransformationPoint(
      "/icons/translateGroup.svg",
    );
    ...
    if (this.__parameters.rotate) {
      this.__rotateHandle.addFloorplanListener(
        "DragStart",
        this.__rotateHandleDragStartEvent,
      );
      this.__rotateHandle.addFloorplanListener(
        "DragMove",
        this.__rotateHandleDragMoveEvent,
      );
      this.__rotateHandle.addFloorplanListener(
        "DragEnd",
        this.__rotateHandleDragEndEvent,
      );
      this.addChild(this.__rotateHandle);
    }
    if (this.__parameters.translate) {
      this.__translateHandle.addFloorplanListener(
        "DragStart",
        this.__rotateHandleDragStartEvent,
      );
      this.__translateHandle.addFloorplanListener(
  ...
  __createScalingHandles() {
    ...
    for (; i < totalHandles; i++) {
      ...
      let handle = new CornerGroupScalePoint(i, {
        move: { x: xMove, y: yMove },
        offset: { x: xOffsets[i], y: yOffsets[i] },
      });
      handle.addFloorplanListener(
        "DragStart",
  ...
  __updateMatrixOfGroup() {
    ...
    this.__currentGroup.applyTransformations(
      scale,
      this.__resizer.rotationRadians,
      this.__toUnits(this.__resizer.origin.clone()),
    );
  ...
  __setControlsPosition() {
    if (this.__parameters.scale) {
      ...
      for (let i = 0; i < this.__scalingHandles.length; i++) {
        ...
        this.__originalPositions.push(
          new Vector2(handle.position.x, handle.position.y),
        );
      }
    }

    if (this.__parameters.rotate) {
      let midPoint = this.__resizer.br
        .clone()
        .sub(this.__resizer.tr)
        .multiplyScalar(0.5)
        .add(this.__resizer.tr);
      let direction = this.__resizer.bl
        .clone()
        .sub(this.__resizer.br)
        .normalize()
        .negate();
    ...
    if (this.__parameters.translate) {
      this.__translateHandle.position.set(
        this.__resizer.center.x,
        this.__resizer.center.y,
      );
    }
  }
  ...
  __updateTransformControls() {
    ...
    this.__resizer = new CornerGroupRectangle(
      this.__floorplan,
      this.__size,
      this.__center,
    );
  ...
  set selected(instanceOfCornerOrWallOrRoom) {
    ...
    if (this.__selected) {
      ...
      if (instanceOfCornerOrWallOrRoom instanceof Corner) {
        ...
        throw new Error(
          "selected  can assigned with a Corner, Wall, Room or null. Unrecogonized datatype",
        );
      }
