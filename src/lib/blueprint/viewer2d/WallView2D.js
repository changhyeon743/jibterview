...
export class WallDimensions2D extends Graphics {
  constructor(floorplan, options, wall) {
    ...
    Configuration.getInstance().addEventListener(
      EVENT_CHANGED,
      this.__measurementUpdateEvent.bind(this),
    );
  }
  ...
  __wallNormal() {
    ...
    let wallDirectionNormalized = new Vector3(
      wallDirectionNormalized2D.x,
      wallDirectionNormalized2D.y,
      0,
    );
  ...
  __drawDimensionLine() {
    ...
    let p1End = this.__wallOffsetLocation(
      this.__wall.start.location.clone(),
      100,
    );

    let p2Start = this.__toPixels(this.__wall.end.location.clone());
    let p2End = this.__wallOffsetLocation(
      this.__wall.end.location.clone(),
      100,
    );

    let p1Center = this.__wallOffsetLocation(
      this.__wall.start.location.clone(),
      50,
    );
    let p2Center = this.__wallOffsetLocation(
      this.__wall.end.location.clone(),
      50,
    );
    ...
    let radius = 5;
    let p4 = p1Center.add(
      wallDirectionNormalized.clone().multiplyScalar(radius),
    );
    let p5 = p2Center.sub(
      wallDirectionNormalized.clone().multiplyScalar(radius),
    );
...
export class Edge2D extends BaseFloorplanViewElement2D {
  ...
  __getCornerCoordinates() {
    ...
    let iStartPoint = Dimensioning.cmToPixelVector2D(
      this.__edge.interiorStart(),
    );
    let iEndPoint = Dimensioning.cmToPixelVector2D(this.__edge.interiorEnd());

    let eStartPoint = Dimensioning.cmToPixelVector2D(
      this.__edge.exteriorStart(),
    );
    ...
    return [
      iStartPoint.add(vectStart.multiplyScalar(0.5)),
      iEndPoint.add(vectEnd.multiplyScalar(0.5)),
    ];
  }
  ...
  __drawEdgeArrow(forEdge, color = 0xdddddd, alpha = 1.0, toEdge = null) {
    let start = Dimensioning.cmToPixelVector2D(
      forEdge.getStart().location.clone(),
    );
    ...
    if (toEdge) {
      let start2 = Dimensioning.cmToPixelVector2D(
        toEdge.getStart().location.clone(),
      );
      let end2 = Dimensioning.cmToPixelVector2D(
        toEdge.getEnd().location.clone(),
      );
    ...
    this.drawRect(
      midPoint.x - arrowSize * 0.5,
      midPoint.y - arrowSize * 0.5,
      arrowSize,
      arrowSize,
    );
    this.endFill();
...
export class WallView2D extends BaseFloorplanViewElement2D {
  ...
  __getCornerCoordinates() {
    let sPoint = Dimensioning.cmToPixelVector2D(
      this.__wall.start.location.clone(),
    );
    let ePoint = Dimensioning.cmToPixelVector2D(
      this.__wall.end.location.clone(),
    );
    return [sPoint, ePoint];
  }
  ...
  __drawInWallItems() {
    ...
    for (let i = 0; i < inWallItems.length; i++) {
      ...
      let up = wallDirection
        .clone()
        .rotateAround(new Vector2(), Math.PI * 0.5)
    ...
    if (this.__isDragging) {
      ...
      if (Configuration.getBooleanValue(directionalDrag)) {
        ...
        if (Configuration.getBooleanValue(snapToGrid) || this.__snapToGrid) {
          let snappedLength =
            Math.floor(
              vectorToProjected.length() /
                Configuration.getNumericValue(snapTolerance),
            ) * Configuration.getNumericValue(snapTolerance);
          vectorToProjected = vectorToProjected
            .normalize()
            .multiplyScalar(snappedLength);
        }
