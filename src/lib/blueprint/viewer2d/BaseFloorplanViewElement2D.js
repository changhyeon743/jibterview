...
export class BaseFloorplanViewElement2D extends Graphics {
  constructor(floorplan, options) {
    ...
    this.on("mousedown", this.__mouseClickEvent).on(
      "touchstart",
      this.__mouseClickEvent,
    );
  ...
  __deactivate() {
    this.off("mousedown", this.__mouseClickEvent).off(
      "touchstart",
      this.__mouseClickEvent,
    );
    this.off("mouseupoutside", this.__mouseUpEvent).off(
      "touchendoutside",
      this.__mouseUpEvent,
    );
    this.off("mouseup", this.__mouseUpEvent).off(
      "touchend",
      this.__mouseUpEvent,
    );
    this.off("mousemove", this.__mouseMoveEvent).off(
      "touchmove",
      this.__mouseMoveEvent,
    );
    this.off("mouseover", this.__mouseOverEvent).off(
      "mouseout",
      this.__mouseOutEvent,
    );
  }
  ...
  __vectorToPixels(v) {
    return new Vector2(
      Dimensioning.cmToPixel(v.x),
      Dimensioning.cmToPixel(v.y),
    );
  }
  ...
  remove() {
    this.off("mousedown", this.__mouseDownEvent).off(
      "touchstart",
    ...
    this.__keyboard.removeEventListener(
      EVENT_KEY_RELEASED,
      this.__keyListenerEvent,
    );
    this.__keyboard.removeEventListener(
  ...
  set selected(flag) {
    ...
    if (flag) {
      this.__eventDispatcher.dispatchEvent({
        type: EVENT_2D_SELECTED,
        item: this,
      });
    } else {
      this.__eventDispatcher.dispatchEvent({
        type: EVENT_2D_UNSELECTED,
        item: this,
      });
    }
  }
