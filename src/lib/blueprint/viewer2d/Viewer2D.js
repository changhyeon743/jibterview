...
export class Viewer2D extends Application {
  constructor(canvasHolder, floorplan, options) {
    ...
    this.__floorplanContainer.position.set(
        this.__canvasHolder.clientWidth * 0.5,
        this.__canvasHolder.clientHeight * 0.5
    );
  ...
  /** */
  // Window resize handler update
  _handleWindowResize() {
    let heightMargin = this.__canvasHolder.offsetTop;
    let widthMargin = this.__canvasHolder.offsetLeft;

    let w = this.__options.resize
        ? this.__canvasHolder.clientWidth - widthMargin
        : this.__canvasHolder.clientWidth;
    let h = this.__options.resize
        ? this.__canvasHolder.clientHeight - heightMargin
        : this.__canvasHolder.clientHeight;
